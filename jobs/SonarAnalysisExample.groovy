job('SonarAnalysisExample') {
  ...
  scm {
    git {
      remote {
        url 'git@github.com:CareerCruising/Xello.Feed.API.git'
  ...
  steps {
    ...
    docker build -t ${IMG_NAME}:${BUILD_NUMBER} -f ./${DOCKER_TEMPLATE} --build-arg SONAR_URL=${SONAR_URL} --build-arg SONAR_TOKEN=${SONAR_TOKEN} --build-arg SONAR_PROJECT_KEY=${SONAR_PROJECT_KEY} --build-arg PROJECT_NAME=${PROJECT_NAME} .'''.stripIndent().trim())
  }
}